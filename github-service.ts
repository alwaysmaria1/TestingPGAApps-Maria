...
import { parseCode, analyzeAST } from "./code-parser-service.js";
...
async function aggregateASTReport(
  ...
  for (const file of changedFilesData) {
    try {
      ...
      // Parse the full content using espree.
      const ast = parseCode(content);

      // Format the AST as JSON.
      const astString = JSON.stringify(ast, null, 2);

      // Analyze the AST to get metrics.
      const metrics = analyzeAST(ast);

      // Combine AST and metrics in the report.
      astReport += `### ${
        file.filename
      }\n\`\`\`\nAST:\n${astString}\n\nMetrics:\n${JSON.stringify(
        metrics,
      ...
      if (error instanceof Error) {
        ...
        astReport += `### ${file.filename}\n\`\`\`\nError processing file: Unknown error occurred.\n\`\`\`\n\n`;
      }
    }
  }
...
export async function handlePullRequestEvent(
  ...
  try {
    ...
    // Call AI code review agent
    let feedback: string = "";

    try {
      feedback = await reviewAgent.generateCommitReview(
        changedFilesData,
        rawCode,
        astReport
      );
      ...
      // console.error("Error generating commit review:", error);
    }
...
export async function handlePullRequestCommentEvent(
  ...
  try {
    ...
    const previousFilesData = await Promise.all(
      changedFilesData.map(async (file) => {
        // Create an object similar to changedFilesData but include previous content.
        const previousContent = await fetchFullFileContent(
          octokit,
          owner,
          repo,
          file.filename,
    ...
    const previousComments = comments
      ...
      .map((comment) => {
        ...
        const ellipsis =
          comment.body?.length && comment.body?.length > 500 ? "..." : "";
